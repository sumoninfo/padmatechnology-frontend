    ...
    <div class="nav ">
      <div class="bg-gray-800 pt-3 ">
        <div
          ...
          <h3 class="font-bold pl-2 text-2xl">Amenities</h3>
          <router-link :to="{name:'adminAmenityCreate'}"
        ...
        <div
          ...
          <table class="min-w-full text-left">
            ...
            <template v-if="amenities.length">
              <tr v-for="(product, index) in amenities">
                ...
                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                  <router-link title="Edit" :to="{ name: 'adminAmenityEdit', params: {id: product.id }}"
          ...
          <pagination v-if="amenities.length > 0" :pagination="pagination" @paginate="getLists()" :offset="5"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getLists() {
      ...
      ApiService.get(`/admin/amenities`, {params: params}).then((res) => {
    ...
    destroy(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete(`/admin/amenities/${id}`,).then(res => {
            this.getLists();
